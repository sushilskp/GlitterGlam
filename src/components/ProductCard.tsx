    ...
    <div className="product-card overflow-hidden flex flex-col justify-between group relative border border-[#C9A66B]/10 bg-white" id={`prod-card-${product.id}`}>
      ...
      <div className="relative aspect-square overflow-hidden bg-stone-100 cursor-pointer" onClick={() => onViewDetails(product)}>
        ...
        <div className="absolute top-2.5 left-2.5 flex flex-col gap-1.5 items-start">
          ...
          <span className={`text-[9px] uppercase tracking-widest px-2.5 py-1 font-bold shadow-sm border ${
          ...
          {/* Discount Tag */}
          {discountPercent > 0 && (
            <span className="bg-red-500 text-white text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-sm shadow-sm flex items-center gap-1">
        ...
        <div className="absolute bottom-2.5 left-2.5">
          {isOutOfStock ? (
            <span className="bg-red-600/90 text-white text-[10px] uppercase font-bold tracking-widest px-2.5 py-1 rounded-sm shadow">
              Sold Out
            </span>
          ) : isLowStock ? (
            <span className="bg-amber-600/95 text-white text-[9px] uppercase tracking-wider px-2 py-1 rounded-md shadow flex items-center gap-1 font-medium">
      ...
      <div className="p-4 flex-1 flex flex-col justify-between space-y-4">
        ...
        {/* Texts */}
        <div className="space-y-1.5 cursor-pointer" onClick={() => onViewDetails(product)}>
          ...
          <h3 className="font-serif text-[15px] sm:text-base text-[#1D1D1D] font-medium leading-snug group-hover:text-[#C9A66B] transition-colors line-clamp-2">
            {product.name}
          </h3>
          ...
          <p className="text-stone-500 text-xs line-clamp-2 font-light italic leading-relaxed pt-1">
            {product.description}
          </p>
        </div>
        ...
        <div>
          <div className="flex items-baseline gap-2.5 pt-2">
            <span className="font-serif text-lg text-[#C9A66B] font-bold">
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
            <button
              ...
              className="bg-[#1D1D1D] text-[#FDFBF8] text-[10px] uppercase font-bold tracking-widest py-3 hover:bg-[#C9A66B] transition-colors cursor-pointer"
            >
              View Piece
            </button>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="bg-[#25D366] text-white hover:bg-[#1DA851] text-[10px] uppercase font-bold tracking-widest py-3 flex items-center justify-center gap-1 transition-all rounded shadow"
            >
